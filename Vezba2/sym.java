public class sym {
    // EOF
    public static final int EOF = 0;
    
    // keywords
    public static final int MAIN = 1;
    public static final int EXIT = 2;
    public static final int INT = 3;
    public static final int FLOAT = 4;
    public static final int BOOL = 5;
    public static final int FOR = 6;
    public static final int IN = 7;
    public static final int APPLY = 8;
    
    // identifiers & constants
    public static final int ID = 9;
    public static final int CONST = 10;
    
    // operators
    public static final int ASSIGN = 11;   // :=
    public static final int PLUS = 12;     // +
    public static final int MINUS = 13;    // -
    
    // punctuation
    public static final int SEMICOLON = 14; // ;
    public static final int COMMA = 15;     // ,
    public static final int LBRACKET = 16;  // [
    public static final int RBRACKET = 17;  // ]
    
    // NOVI NETERMINALI za LL(1) parser
    public static final int APPLY_EXPR = 18;
    public static final int NAMELIST = 19;
    public static final int NAMELIST_P = 20;
    public static final int EXPRESSION = 21;
    public static final int EXPRESSION_P = 22;
    public static final int TERM = 23;
    
    // Maksimalan broj simbola (terminali + neterminali)
    public static final int MAX_SYMBOLS = 40;
}
