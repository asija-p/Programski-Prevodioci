...
{cifra}+"."{cifra}*([Ee][+-]?{cifra}+)? { return new Symbol(sym.FLOATCONST, new Float(yytext())); }
"."{cifra}+([Ee][+-]?{cifra}+)?         { return new Symbol(sym.FLOATCONST, new Float(yytext())); }
{charconst}                  { return new Symbol(sym.CHARCONST, new Character(yytext().charAt(1))); }
